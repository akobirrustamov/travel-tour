      ...
      if (res && !res.error) {
        ...
        const slides = res.data.map((slide, index) => ({
          ...
          background: slide.media
            ? `${baseUrl}/api/v1/file/getFile/${slide.media.id}`
            : null,
          ...
          description_uz:
            ...
            "Biz bilan dunyoning eng go'zal joylarini kashf eting",
          description_ru:
            slide.description_ru ||
            "Откройте для себя самые красивые места мира с нами",
          description_en:
            slide.description_en ||
            "Discover the world's most beautiful places with us",
          description_turk:
            slide.description_turk ||
            "Dünyanın en güzel yerlerini bizimle keşfedin",
        }));
