...
function BookingsTable() {
  ...
  const loadRoomTypes = async () => {
    const res = await ApiCall("/api/v1/room/type", "GET");
    if (res && !res.error) {
      setRoomTypes(res.data);
      console.log(res.data);
    } else {
      toast.error("Ошибка при загрузке типов комнат");
    }
  };
  useEffect(() => {
    loadData();
  ...
  const handleSaveRoom = async () => {
    ...
    try {
      ...
      if (
        ...
        selectedRoom.roomType.id !== selectedRoom.originalTypeId
      ) {
        const typeRes = await ApiCall(
          `/api/v1/room/type/${selectedRoom.id}/${selectedRoom.roomType.id}`,
          "PUT"
        );
    ...
    <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="space-y-4">
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <p className="text-gray-600">
                Исходный тип:{selectedRoom.description}
              </p>
            </div>
      ...
      {showEditModal && selectedBooking && (
        <ModernModal
          ...
          title={isReception ? "Информация о брони" : "Редактировать бронь"}
          animation={modalAnimation}
        >
          {isReception ? (
            <div className="space-y-6">
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap md:flex-nowrap gap-3 justify-between pt-6 border-t border-slate-200">
                <button
                  onClick={handleDeleteBooking}
                  className="w-full md:w-fit px-6 py-3 bg-gradient-to-r from-rose-500 to-rose-600 text-white rounded-xl hover:from-rose-600 hover:to-rose-700 transition-all duration-200 font-semibold"
                >
                  🗑️ Удалить бронь
                </button>
                <div className="flex gap-3 w-full md:w-fit">
                  <button
                    ...
                    className="w-1/2 md:w-fit px-8 py-3 bg-slate-100 text-slate-700 rounded-xl hover:bg-slate-200 transition-all duration-200 font-medium"
                  >
                    Отмена
                  </button>
                  <button
                    onClick={handleSaveEdit}
                    className="w-full md:w-fit px-8 py-3 bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded-xl hover:from-blue-600 hover:to-indigo-600 transition-all duration-200 font-semibold shadow-lg hover:shadow-xl"
                  >
                    💾 Сохранить
                  </button>
                </div>
