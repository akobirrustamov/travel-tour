...
function AdminTour() {
    ...
    const fetchTours = async () => {
        try {
            setLoading(true);
            const res = await requestWithRefresh(`/api/v1/travel-tours/page?page=${currentPage}&size=12`);
            if (res && !res.error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const url = editingId ? `/api/v1/travel-tours/${editingId}` : "/api/v1/travel-tours";
    ...
    const handleDelete = async (id) => {
        if (!window.confirm("Are you sure you want to delete this travel tour?")) return;

        try {
            const res = await requestWithRefresh(`/api/v1/travel-tours/${id}`, "DELETE");
            if (res && !res.error) {
    ...
    const handleToggleActive = async (tour) => {
        try {
            ...
            const res = await requestWithRefresh(`/api/v1/travel-tours/${tour.id}`, "PUT", updatedData);

            if (res && !res.error) {
                toast.success(`Tour ${!tour.active ? 'activated' : 'deactivated'} successfully`);
                fetchTours();
            } else {
    ...
    const getDisplayTitle = (tour) => {
        switch(selectedLanguage) {
            case "uz": return tour.title_uz || tour.title_ru || tour.title_en || tour.title_turk || "No title";
            case "ru": return tour.title_ru || tour.title_uz || tour.title_en || tour.title_turk || "No title";
            case "en": return tour.title_en || tour.title_uz || tour.title_ru || tour.title_turk || "No title";
            case "turk": return tour.title_turk || tour.title_uz || tour.title_ru || tour.title_en || "No title";
            default: return tour.title_uz || tour.title_ru || tour.title_en || tour.title_turk || "No title";
        }
    };
    ...
    const getDisplayDescription = (tour) => {
        switch(selectedLanguage) {
            case "uz": return tour.description_uz || tour.description_ru || tour.description_en || tour.description_turk || "";
            case "ru": return tour.description_ru || tour.description_uz || tour.description_en || tour.description_turk || "";
            case "en": return tour.description_en || tour.description_uz || tour.description_ru || tour.description_turk || "";
            case "turk": return tour.description_turk || tour.description_uz || tour.description_ru || tour.description_en || "";
            default: return tour.description_uz || tour.description_ru || tour.description_en || tour.description_turk || "";
        }
    };
    ...
    const renderLanguageBadges = (tour) => {
        const badges = [];
        if (tour.title_uz?.trim()) badges.push({ key: "uz", label: "UZ", color: "blue" });
        if (tour.title_ru?.trim()) badges.push({ key: "ru", label: "RU", color: "green" });
        if (tour.title_en?.trim()) badges.push({ key: "en", label: "EN", color: "purple" });
        if (tour.title_turk?.trim()) badges.push({ key: "turk", label: "TR", color: "yellow" });
    ...
    const renderTitleInputs = () => {
        const titleInputs = [
            { key: "uz", label: "O'zbekcha", placeholder: "Sayoxat nomi", required: true },
            { key: "ru", label: "Русский", placeholder: "Название тура", required: false },
            { key: "en", label: "English", placeholder: "Tour title", required: false },
            { key: "turk", label: "Türkçe", placeholder: "Tur başlığı", required: false }
        ];
                ...
                {titleInputs.map((lang) => (
                    <div key={lang.key} className="space-y-2">
                        <label className="block text-sm font-medium text-gray-700">
                            {lang.label} Title {lang.required && <span className="text-red-500">*</span>}
                        </label>
                        <input
                            type="text"
    ...
    const renderItineraryEditor = () => {
        return (
            <div className="space-y-2">
                <label className="block text-sm font-medium text-gray-700">
                    Itinerary Details
                    <span className="ml-1 text-xs text-gray-500">(Day-by-day tour plan)</span>
                </label>
                <div className="border border-gray-300 rounded-lg overflow-hidden">
                    <MyCkeditor
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-10">
                        <div>
                            ...
                            <p className="text-gray-600 mt-2">Manage your travel tours with multi-language support</p>
                        </div>
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6 mb-10">
                                {filteredTours.map((tour) => {
                                    ...
                                    const thumbnail = tour.images && tour.images.length > 0
                                        ? `${baseUrl}/api/v1/file/getFile/${tour.images[0].id}`
                                        : null;

                                    return (
                                        <div key={tour.id} className="group bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2">
                                            {/* Image */}
                                            ...
                                            <div className="p-5">
                                                ...
                                                <div className="flex items-start justify-between mb-3">
                                                    ...
                                                    >
                                                        {tour.active ? <Eye size={16} /> : <EyeOff size={16} />}
                                                    </button>
                                                </div>

                                                {/* Cities */}
                                                <div className="flex items-start mb-3">
                                                    <MapPin size={14} className="mr-1 text-gray-500 mt-0.5" />
                            ...
                            {filteredTours.length === 0 && !loading && (
                                <div className="text-center py-16 bg-white rounded-2xl shadow-sm border border-gray-100">
                                    ...
                                    <h3 className="text-2xl font-bold text-gray-800 mb-3">No travel tours found</h3>
                            ...
                            {totalPages > 1 && (
                                <div className="flex flex-col sm:flex-row items-center justify-between bg-white p-6 rounded-2xl shadow-sm border border-gray-100 mt-8">
                                    <div className="text-sm text-gray-700 mb-4 sm:mb-0">
                                        Showing <span className="font-bold text-emerald-600">{filteredTours.length}</span> of{" "}
                                        <span className="font-bold text-gray-800">{totalItems}</span> tours • Page{" "}
                                        <span className="font-bold text-emerald-600">{currentPage + 1}</span> of{" "}
                    ...
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-7xl max-h-[95vh] overflow-y-auto">
                            ...
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-gray-600 mt-1">
                                        {editingId ? "Update tour information and details" : "Add a new travel tour with multi-language support"}
                                    </p>
                                </div>
                                <button
                                    onClick={() => {
                        ...
                        <form onSubmit={handleSubmit} className="p-8 space-y-8">
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center justify-between">
                                    <h3 className="text-xl font-bold text-gray-800">Tour Titles</h3>
                            ...
                            {/* Tour Information */}
                            <div className="space-y-4">
                                <h3 className="text-xl font-bold text-gray-800">Tour Information</h3>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <p className="text-xs text-gray-500">Inactive tours won't be shown on website</p>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div>
                                    <div className="mb-4">
                                        <p className="text-sm text-gray-600 mb-2">
                                            Recommended: 1200x800px, max 5MB per image (JPEG, PNG, WebP)
                                        </p>
                            ...
                            {/* Descriptions */}
                            <div className="space-y-4">
                                <h3 className="text-xl font-bold text-gray-800">Tour Descriptions</h3>
