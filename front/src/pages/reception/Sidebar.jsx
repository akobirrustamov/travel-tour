...
function Sidebar({ onLogout }) {
  ...
  return (
    <aside className="w-64 bg-gray-800 text-white p-6 flex flex-col justify-between min-h-screen">
      <div>
        <h2 className="text-2xl font-bold mb-8">Reception Panel</h2>
        <nav className="space-y-2">
          <button
            onClick={() => navigate("/reception/dashboard")}
            className={getButtonClass("/reception/dashboard")}
          >
            🏠 Bosh sahifa
          </button>
          ...
          <button
            ...
            className={getButtonClass("/reception/bookings")}
          >
            📅 Bookings
          </button>

          <button
            onClick={() => navigate("/reception/talk")}
            className={getButtonClass("/reception/talk")}
          >
            💬 Chats
          </button>
        </nav>
