...
function Dashboard() {
  ...
  const getPeopleCount = (roomTypeName) => {
    if (!roomTypeName) return 1;
    const type = roomTypeName.toLowerCase();
    if (type.includes("twin") || type.includes("double")) return 2;
    if (type.includes("triple")) return 3;
    return 1;
  };

  // ✅ общее количество человек
  const totalPeople = bookings.reduce((acc, b) => {
    const type = b.room?.roomType?.name || "";
    return acc + getPeopleCount(type);
  }, 0);
              ...
              <table className="min-w-full border border-gray-200 text-sm text-center">
                ...
                <tbody>
                  {bookings.map((b) => {
                    ...
                    const peopleCount = getPeopleCount(roomType);
                    return (
